...
describe("WordItem 에서", () => {
  test("단어 설정이 올바르게 되어야 한다.", () => {
    const wordItem = new WordItem({ text: "world", second: 10 });
    expect(wordItem.value).toEqual({
      text: "world",
      second: 10,
      isPassed: void 0,
      timeConsumed: 0,
  ...
  test("지정된 시간이 지나면 유효하지 않은 word로 판정되야 한다.", async () => {
    const wordItem = new WordItem({ text: "world", second: 1 });
    wordItem.start();
    await sleep(2);
  ...
  test("start 하지 않았다면 time이 줄어들지 않아야 한다.", async () => {
    const wordItem = new WordItem({ text: "world", second: 5 });
    await sleep(2);
  ...
  test("정답 성공/실패에 따른 정보를 저장해야한다.", async () => {
    const wordItem1 = new WordItem({ text: "world", second: 4 });
    wordItem1.start();
    wordItem1.pass(true);
    expect(wordItem1.value.isPassed).toBeTruthy();

    const wordItem2 = new WordItem({ text: "world", second: 4 });
    wordItem2.start();
  ...
  test("정답을 맞출 때 까지 사용된 시간을 올바르게 저장해야한다.", async () => {
    const wordItem = new WordItem({ text: "world", second: 4 });
