...
describe('WordItemQueue 에서', () => {
  test('get()이 올바른 값을 리턴해야 한다.', () => {
    ...
    expect(wordItemQueue.dequeue().value).toEqual(new WordItem({ text: 'hello', second: 10 }).value);
    expect(wordItemQueue.dequeue().value).toEqual(new WordItem({ text: 'world', second: 11 }).value);
